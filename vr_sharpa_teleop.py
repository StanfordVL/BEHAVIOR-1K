...
# Explicit startup pose: Franka home (matches what the URDF puts the arm in
# when no override is given) + all 22 finger joints at zero (straight hand).
# Without this, gravity drifts low-effort fingers off-zero before the position
# drive can react — visible at startup as the bent ring/pinky fingers.
# Equivalent to MuJoCo's <key name="home" qpos="..."/> keyframe.
...
# Per-joint effort multipliers. Diagnostics confirmed:
#  - URDF axes correct, but URDF effort values are too low for sim gravity.
#  - pinky_CMC needs to outweigh the reaction torque from pinky_MCP_FE flexing
#    (10.57 N·m vs 37.28 N·m at 20× → CMC loses, runs to +4.2 rad).
#  - MCP_FE joints sag past their lower limits at rest under gravity.
# Boost the weak ones individually instead of globally over-driving everything
# (which previously caused upper-limit overshoots on the strong joints).
# Per-joint effort multipliers.
#
# Pinky: STARVED of effort intentionally. The pinky chain has a feedback
# instability — boosting pinky_MCP_FE creates a large reaction torque on
# pinky_MC, which spins pinky_CMC even when pinky_CMC has its own boost.
# Spinning pinky_CMC then "wraps" pinky_MCP_FE's joint angle, and the
# actuator just keeps trying to correct, feeding more torque into the
# loop. Verdict: don't fight gravity on the pinky. Use 4 fingers + thumb
# for grip; pinky just hangs wherever gravity puts it.
_PINKY_CMC_BOOST      =   1.0
_PINKY_MCP_FE_BOOST   =   1.0
_PINKY_MCP_AA_BOOST   =   1.0
_PINKY_PIP_BOOST      =   1.0
# Other fingers: moderate boosts that let MCP_FE flex without overshooting.
...
def main():
    ...
    if args.no_vr:
        ...
        _test_hand = False
        # Slower cycles: gives the compliant PD drive time to settle, mimics
        # the smoothing pipeline used in the MuJoCo teleop (alpha=0.4, sigma=2,
        # 3-point spline). Was 200/100; now 360/180 (~3s ramp at 60Hz).
        _CYCLE_FRAMES = 360
        _RAMP_FRAMES = 180
        if args.test_hand:
            ...
            # Open = straight hand (zeros, not lower URDF limit which would
            # be hyperextended). Close = 92% of upper limit, NOT the limit
            # itself — gives PhysX margin so it never has to fight a hard
            # stop. AA joints stay at 0 in both poses; DIPs follow open.
            ...
            # Skip pinky in grip — chain runaway between pinky_CMC and pinky_MCP_FE
            # is unstable across effort tunings. Real grasps work fine with 4 fingers
            # + thumb. Pinky indices in the gripper command vector: 17..21.
            for _pinky_idx in [17, 18, 19, 20, 21]:
                _hand_close[_pinky_idx] = 0.0
                _hand_open[_pinky_idx] = 0.0

            # Per-joint effort. Default 20× for most joints; bigger for the
            # pinky chain (which loses to MCP_FE chain torque at 20×) and
            # for any MCP_FE that drifts past its lower limit at rest.
            # Diag confirmed URDF axes are correct; this is a runtime force
            # imbalance, not a model bug.
            ...
            if _gripper_ctrl is not None:
                ...
                _log(f"  [TEST] Per-joint effort applied (default 20×, pinky_CMC 150×, pinky_MCP_FE 60×)")
    ...
    vrsys = OVXRSystem(
        ...
        system="OpenXR",  # Isaac Sim 5.1 dropped the SteamVR backend; OpenXR routes via active_runtime.json
    ...
    # Slower ramp (~1.5s @120Hz). Was 0.4s — too aggressive for stable contact.
    ...
    from omnigibson.robots.franka_mounted_sharpa import SHARPA_JOINT_LIMITS
    # Open = straight (zeros). Close = 92% of upper URDF limit, NOT the limit
    # itself — gives PhysX margin so it never holds joints at hard stops.
    # AA joints stay 0 in both poses; DIPs follow open.
    _hand_open = th.zeros(22)
    _hand_close = SHARPA_JOINT_LIMITS[:, 1].clone() * 0.85
    ...
    # Skip pinky entirely — chain runaway between pinky_CMC and pinky_MCP_FE
    # is unstable across effort tunings. Pinky indices in the 22-element
    # gripper command vector: 17..21. Grip uses 4 fingers + thumb.
    for _pinky_idx in [17, 18, 19, 20, 21]:
        _hand_close[_pinky_idx] = 0.0
        _hand_open[_pinky_idx] = 0.0
    ...
    if _gripper_ctrl is not None:
        # Per-joint effort (see _build_per_joint_effort docstring for rationale).
        _PER_JOINT_EFFORT = _build_per_joint_effort(20.0, robot.dof_names_ordered)
        _finger_dof_idx = ControllerView.get_dof_idx(_gripper_ctrl[0])  # robot.controllers[name] = (group_key, controller_idx)
        _effort_limits = robot.control_limits["effort"]
        for dof in _finger_dof_idx:
            ...
            _effort_limits[1][dof] *= mult
            robot.joints[jname].max_effort = robot.joints[jname].max_effort * mult
        print(f"[Grasp] Per-joint effort applied; pinky boosted 100-150×")
        log(f"[Grasp] Per-joint effort applied; pinky boosted 100-150×")
