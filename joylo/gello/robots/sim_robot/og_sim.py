...
class OGRobotServer:
    ...
    def _update_observations(self) -> Dict[str, th.tensor]:
        ...
        obs["base_contact"] = RigidContactAPI.is_in_contact(self.env.scene.idx, set(self.robot.non_floor_touching_base_links), with_set=None, ignore_set=None, current=False) if INCLUDE_BASE_CONTACT_OBS else False
        obs["trunk_contact"] = RigidContactAPI.is_in_contact(self.env.scene.idx, set(self.robot.trunk_links), with_set=None, ignore_set=None, current=False) if INCLUDE_TRUNK_CONTACT_OBS else False
        ...
        for i, arm in enumerate(self.robot.arm_names):
            ...
            obs[f"arm_{arm}_contact"] = RigidContactAPI.is_in_contact(self.env.scene.idx, set(self.robot.arm_links[arm]), with_set=None, ignore_set=None, current=False) if VIEWING_MODE != ViewingMode.VR and INCLUDE_ARM_CONTACT_OBS else False
