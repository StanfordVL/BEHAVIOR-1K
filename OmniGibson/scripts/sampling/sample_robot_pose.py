...
def sample_robot_poses(env) -> Dict[str, List[Dict]]:
    ...
    """
    # This script samples a single template instance at a time, so everything below indexes scene 0.
    # (env.scene further down enforces this too, but only after the object_scopes lookups.)
    assert env.num_envs == 1, f"sample_robot_poses only supports single-env environments, got num_envs={env.num_envs}"
    ...
    for cond in env.task.activity_initial_conditions:
        ...
        for bddl_name in cond.get_relevant_objects():
            obj = env.task.object_scopes[0].get(bddl_name, None)
            if obj is not None:
    ...
    for i, tro_file in enumerate(task_info["tro_files"]):
        ...
        for bddl_name, obj_state in tro_torch_state.items():
            if "agent" in bddl_name or "robot_poses" in bddl_name:
                continue
            env.task.object_scopes[0][bddl_name].load_state(obj_state, serialized=False)
        for _ in range(25):
            og.sim.step_physics()
            for bddl_name, entity in env.task.object_scopes[0].items():
