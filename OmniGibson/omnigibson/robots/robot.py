            ...
            for obj in self.scene.objects:
                if obj.prim_type == PrimType.RIGID:
                    for link in obj.links.values():
                        if isinstance(link, RigidPrim) and link.contact_reporting_enabled:
                            scene_body_filters.append(link.prim_path)

            if len(finger_paths) > 0 and len(scene_body_filters) > 0:
        ...
        if self.grasping_mode != "physical":
            ...
            if is_grasping == IsGraspingState.TRUE and candidate_obj is not None:
                ...
                if self._contact_view is not None:
                    ...
                    nonzero_indices = th.nonzero(impulses > 0)
                    for idx in nonzero_indices:
                        row, col = (idx[1].item(), idx[2].item()) if len(idx) == 3 else (idx[0].item(), idx[1].item())
                        other_contact = self._contact_view_row_idx_to_path.get(row)
                        link_contact = self._contact_view_col_idx_to_path.get(col)
            ...
            if row_idx is not None:
                for finger_path in finger_paths:
                    ...
                    if col_idx is not None:
                        ...
                        c_counts = contact_counts.squeeze(0) if contact_counts.ndim == 3 else contact_counts
                        s_indices = start_indices.squeeze(0) if start_indices.ndim == 3 else start_indices
                        pts = points.squeeze(0) if points.ndim == 3 else points
                        count = c_counts[row_idx, col_idx].item()
                        if count > 0:
                            start_idx = s_indices[row_idx, col_idx].item()
                            # Simply use the first contact point
                            if start_idx < len(pts):
                                contact_pos_world = th.as_tensor(pts[start_idx], dtype=th.float32)
                                break
