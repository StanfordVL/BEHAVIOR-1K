...
(e.g. the gr00t ``scripts/b1k/serve_b1k.py`` server). For each test instance of a
...
This belongs on the OmniGibson side: run it inside the OmniGibson (e.g.
``behavior``) conda env, while the GR00T policy server runs in its own env.

Example:
    python -m omnigibson.eval.run_eval \
        --task-name turning_on_radio \
        --host 127.0.0.1 --port 8000 \
        --instance-indices 0 --max-steps 50 \
        --output-dir /tmp/b1k_eval --write-video
"""
