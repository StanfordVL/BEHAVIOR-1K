    ...
    def _update(self):
        ...
        contacting_paths = {
            ...
            for this_contact, other_contact in RigidContactAPI.get_contact_pairs(
                scene_idx=scene_idx, query_set=my_link_paths, with_set=None, current=False
            )
                ...
                if child_link is None:
                    ...
                    if not check_physics_stability:
                        ...
                        success = not RigidContactAPI.is_in_contact(
                            scene_idx=self.obj.scene.idx, query_set=[self.obj], with_set=None, ignore_set=[other], current=False
                        )
                        if success:
                            return True
                        else:
