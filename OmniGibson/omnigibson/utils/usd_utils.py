...
class RigidContactAPIImpl:
    ...
    def __init__(self):
        ...
        # Position change tolerance for deciding whether a pair should be updated
        self._REL_POS_EPS = 1e-6
    ...
    def initialize_view(self):
        ...
        with suppress_omni_log(channels=["omni.physx.tensors.plugin"]):
            for scene_idx, _ in enumerate(og.sim.scenes):
                ...
                assert set(sensor_paths) == set(scene_body_filters), (
                    f"RigidContactAPI expected square contact view over {len(scene_body_filters)} rigid bodies, "
                    f"but got sensors={len(sensor_paths)}"
                )
                ...
                assert set(view_prim_paths) == set(scene_body_filters), (
                    f"RigidContactAPI rigid body view mismatch; expected {len(scene_body_filters)} bodies, "
                    f"got {len(view_prim_paths)}"
                )
    ...
    def update_contact_cache(self):
        ...
        for scene_idx in self._CONTACT_VIEW:
            ...
            prev_rel_positions = self._REL_POSITIONS.get(scene_idx, None)
            if prev_rel_positions is None:
                changed_pairs = th.ones(rel_positions.shape[:2], dtype=th.bool)
            else:
                changed_pairs = th.any(th.abs(rel_positions - prev_rel_positions) > self._REL_POS_EPS, dim=-1)

            cached_impulses = self._CONTACT_MATRIX[scene_idx]
    ...
    def clear(self):
        """
        Clears the internal contact caches
        """
        self._CONTACT_MATRIX = dict()
        self._REL_POSITIONS = dict()
        self._PAIRWISE_CONTACT_DATA = dict()
