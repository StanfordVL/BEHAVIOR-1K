...
```
The scene is read automatically from `task_custom_lists.json`. After this command, you should see 1 file generated under `datasets/2026-challenge-task-instances/scenes/SCENE_NAME/json`, with name like `house_double_floor_lower_task_picking_up_trash_0_0_template-partial_rooms.json`.

5. Postprocess the file generated by first step, by adding in other static scene objects.

```sh
python postprocess_sampled_task.py -t TASK_NAME
```
...
6. Randomly generate 300 instances for your task
```sh
...
7. Presample poses for all supported robots
```sh
python sample_robot_pose.py -t TASK_NAME
```

8. To verify whether your sampling is successful, specify your task name in OmniGibson/omnigibson/configs/r1pro_behavior.yaml and run 
```sh
