...
class NavigationController:
    """Navigation utilities on top of scene traversability maps."""
    ...
    def set_room_filter(self, room_ins_id: int | None):
        ...
        `room_ins_id=0`/`None` disables filtering.
        """
    ...
    def find_approach_point(
        self,
        object_pos_2d: th.Tensor,
        approach_dist: float = 0.25,
    ) -> tuple[th.Tensor | None, bool]:
        ...
        max_dist_px = int((approach_dist + 0.3) / resolution)

        h, w = floor_map.shape

        from collections import deque
        visited = set()
        queue = deque([(obj_r, obj_c, 0)])
    ...
    def navigate_to_target(self, target_pos_2d: th.Tensor, approach_dist: float = 0.5,
        ...
        for i, waypoint in enumerate(path):
            ...
            success, obs, acts = self._rotate_to_face(
                wp_x,
                wp_y,
                self.ANGLE_THRESHOLD,
                max_steps=300,
            ...
            success, obs, acts = self._drive_to_point(
                wp_x,
                wp_y,
                self.DIST_THRESHOLD,
                max_steps=200,
