...
#SBATCH --job-name=convert_ai2thor_scenes
#SBATCH --output=/home/cgokmen/projects/BEHAVIOR-1K/slurm/logs/convert_ai2thor_scenes-%A_%a.log
#SBATCH --error=/home/cgokmen/projects/BEHAVIOR-1K/slurm/logs/convert_ai2thor_scenes-%A_%a.log
#SBATCH --array=0-255
...
# --- Configuration ---
SCRIPT_NAME="convert_ai2thor_scenes"
NUM_JOBS=${1:-8}
TOTAL_JOBS_IN_ARRAY=$((NUM_JOBS * SLURM_ARRAY_TASK_COUNT))
DATASET_ROOT="/home/cgokmen/projects/BEHAVIOR-1K/datasets"
SCENE_DIR="/checkpoint/clear/cgokmen/procthor/ai2thor/ai2thor-hab/configs/scenes"
SUCCESS_DIR="${DATASET_ROOT}/ai2thor/jobs"
...
manage_process() {
  ...
  while [ ! -f "${success_file}" ]; do
    ...
    python -u -m omnigibson.examples.scenes.convert_habitat_scenes_to_b1k \
      --dataset-name ai2thor \
      --scene-dir "${SCENE_DIR}" \
      --scene-glob "**/*.scene_instance.json" \
