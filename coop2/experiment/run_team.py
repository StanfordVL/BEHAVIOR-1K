...
def run_team_experiment(
    ...
    """
    Run experiment with one LLM per team of robots.
    ...
    """
    print("="*80)
    print("TEAM TOPOLOGY: ONE LLM PER TEAM OF ROBOTS")
    print("="*80)
    ...
    output_dir = os.path.join(
        results_root,
        f'team_agents{n_agents}_{repair_label}_seed{seed}_{timestamp}',
    )
    ...
    # This topology is defined by its teams, so a layout is required rather than
    # optional. --team-size is the shorthand for "N identical robots in teams of
    # k", which is the common case and does not deserve a JSON file.
    if team_config:
        team_layout = load_team_layout(team_config)
        n_agents = team_layout.n_agents
    else:
        team_layout = homogeneous_layout(
            n_agents, room=room, team_size=team_size or n_agents
        )
    print(f"\n[layout] {team_config or f'--agents {n_agents} --team-size {team_size or n_agents}'}"
    ...
    import coop2.experiment.run_team as _self
    ...
    print(f"\nCreating team topology with {n_agents} agents...")
    print(f"  {len(team_layout.teams)} team(s), one LLM each; a team plans for all "
          f"its robots in one call and regroups only when all of them are done")
    ...
    plan_env = PlanningEnvWrapper(
        ...
        # A message interrupts every member of the team it is addressed to, and
        # the brain answers resume-or-replan for each of them in one call.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run one-LLM-per-team experiment")
    ...
    parser.add_argument("--team-size", type=int, default=None, metavar="K",
                        help="Group --agents robots into teams of K, one LLM each. "
